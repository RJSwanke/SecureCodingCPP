...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps) {
    ...
    T maxLimit = std::numeric_limits<T>::max(); //store max limit for datatype
        
    T minLimit = std::numeric_limits<T>::min(); //store min limit for datatype
    
    for (unsigned long int i = 0; i < steps; ++i) {

        if (maxLimit - increment < result) {  //check for overflow
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps) {
    ...
    T minLimit = 0;  //store min limit for datatype
    
    for (unsigned long int i = 0; i < steps; ++i) {

        if (minLimit + decrement > result) {  //check for underflow
...
//  The solution must work for all of the data types used to call test_overflow() in main().
    ...
    if (result == std::numeric_limits<T>::min()) {
        ...
        std::cout << "Overflow occured while adding.\n";
        // Display a message
    }
    else {
        std::cout << +result << std::endl;
...

{
 ...
 //  The solution must work for all of the data types used to call test_overflow() in main().
    ...
    if (result == std::numeric_limits<T>::max()) {
        // Check for underflow by picking up signal from subtract_numbers
        std::cout << "Underflow occured while subtracting.\n";
        // Display a message
    }
    else {
        std::cout << +result << std::endl;
